...
All 131 members in AoE2ScenarioParser 0.8.4, dumped by `scripts/dump_signatures.py`. Terrain types. A plain `IntEnum` — no `.ID` suffix.

```py
from AoE2ScenarioParser.datasets.terrains import TerrainId
```

Listed as `NAME = value`. Pass the member directly: `TerrainId.<NAME>`.
