...
def _emit_triggers(
    scenario, baselines: Baselines, chunk_size: int
) -> tuple[list[str], str, int, int, int]:
    """Return (part sources, package __init__ source, conditions, effects, variables).
    ...
    init.append("")
    init.append("#: (variable_id, name) for every declared trigger variable. Ids are emitted")
    init.append("#: explicitly: conditions and effects address a variable by id, not by name.")
    variables = [(v.variable_id, v.name) for v in tm.variables]
    init.append("VARIABLES: list[tuple[int, str]] = [")
    for variable_id, name in variables:
        init.append(f"    ({variable_id}, {name!r}),")
    init.append("]")
    init.append("")
    init.append("")
    init.append("def apply(ctx) -> None:")
    init.append('    """Declare the variables, recreate every trigger, restore display order."""')
    init.append("    tm = ctx.tm")
    init.append("    for variable_id, name in VARIABLES:")
    init.append("        tm.add_variable(name, variable_id)")
    ...
    init.append("")
    return parts, "\n".join(init), conditions, effects, len(variables)
...
def decompile(scenario, out_dir: Path, *, source_label: str = "", chunk_size: int = 250) -> DecompileReport:
    ...
    parts, init_src, conditions, effects, variables = _emit_triggers(
        scenario, baselines, chunk_size
    )
