...
| [`evolution_alpha`](modes/evolution_alpha/) | code-defined | **CBA Hero: Ascendants v1.0.1.0** — evolved from the sole v1.0.3 historical baseline, with XS-validated Castle-owner detection, native effects, compact participant-only Objectives, and resource-translated XS ownership for closed-slot and shuffled lobbies, Castle-bound army spawns, independent six-level Sheep and Penguin controls confined to separate tracks with snowy HOLD/OFF pads, side-wall removal and an active 200-warning/220-wall wipe that preserve the front and University barriers, one-shot army/hero/builder orders (returning-pad collision still tracked), Castle-front Hero minimum range, protected reward pads and vote markers, color-independent training restrictions, buildable milestone shores, unobstructed hero spawns, resignation cleanup, mirrored anti-Trebuchet protection, six hero tiers, protected team routes, center rewards, vote-kicks, and live K/D/R. No obsolete corner clutter, blocking Castle Hay, or hidden Goth Palisade bonus. Victory resolves from map state with a Castle-row fallback, and victory waits for defeated-owner cleanup; the shared training ban is derived from the civilization spawn table. 3,903 triggers, 956 units; warning-free strict audit and serialized runtime regression coverage; live P1/P3/P5/P8 initialization and compact Objectives verified. |
...
Ascendants (`modes/evolution_alpha`) has its own documentation set — start with the gameplay or architecture doc depending on what you are here for:

- [`docs/ascendants-gameplay.md`](docs/ascendants-gameplay.md) — the mode as played: sliders, hero tiers, free economy, rewards, walls, vote-kick, victory. No code.
- [`docs/ascendants-architecture.md`](docs/ascendants-architecture.md) — the two source layers, the build pass order, and the fail-closed assertions.
- [`docs/ascendants-xs-runtime.md`](docs/ascendants-xs-runtime.md) — the generated XS runtime, the trigger-variable bridge, and the scenario-color vs lobby-slot identity rule.
- [`docs/ascendants-data-tables.md`](docs/ascendants-data-tables.md) — the civilization tables, hero milestones, the full 0–144 variable registry, and runbooks for changing them.
- [`docs/ascendants-map.md`](docs/ascendants-map.md) — the eight-way transform, the V2 map pass, territory anatomy, and how to read the map report.
- [`docs/ascendants-testing.md`](docs/ascendants-testing.md) — what the build assertions, the test suite, and the audit each prove, and where the engine boundary is.
- [`docs/ascendants-development.md`](docs/ascendants-development.md) — the sole v1.0.3 historical baseline, v1.0.19 candidate, verification layers, and safe issue-fixing loop.
- [`docs/ascendants-control-map.md`](docs/ascendants-control-map.md) — exact color identities, Sheep/Penguin lanes, Castle rows, creation pads, and army/Hero destinations.
