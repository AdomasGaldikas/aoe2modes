...
| --- | --- |
| `_assert_variable_ids_are_contiguous` | A variable block collided, duplicated or left a hole in ids 0–144 |
...
`tests/test_evolution_alpha.py` holds 83 test functions (count them with
...
- `keeps_xs_spawn_and_trigger_routes_in_separate_identity_domains` separates native
  trigger selectors from XS API indices.
- Four `identity_*` tests execute emitted resolver code and serialized resource
  bridges: all 255 nonempty occupied-color subsets in two seat orders, independent
  trigger-selector permutations, delayed/invalid tokens, and persistent binding.
- `native_hud_and_cleanup_work_with_no_xs_identity` starts participation at zero,
  runs actual owner detectors, checks native HUD activation/value reads, and requires
  owner cleanup and victory with every XS identity binding still zero. This closes
  the old harness gap: pre-filled occupancy hid the reported P7/P8 failure.
- The matrix includes the reported P1/P3 vs P5/P6/P7/P8 game, with two lobby slots
  explicitly closed. Closed lobby numbers are not necessarily closed color numbers.
  These tests do not execute DE's native lobby mapping.
